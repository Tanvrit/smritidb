// Persistence adapters per SPEC.md §6 — the browser-safe core.
//
...
//
// This module deliberately contains NO Node-only adapters: `./index.ts` adds
// `fsAdapter` + `sqliteAdapter` on top, and the package's `browser` export
// condition points at an entry built from this file instead, so a bundler
// targeting the web never has to resolve `node:fs/promises` or
// `better-sqlite3`.
