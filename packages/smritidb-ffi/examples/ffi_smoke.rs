...
fn main() {
    ...
    println!(
        "sim(a, a) = {:.4}",
        similarity(a.clone(), a.clone()).unwrap()
    );
    println!(
        "sim(a, b) = {:.4}",
        similarity(a.clone(), b.clone()).unwrap()
    );
    let bound = bind(a.clone(), b.clone()).unwrap();
    let recovered = unbind(bound, b.clone()).unwrap();
    println!(
        "bind/unbind round-trip sim = {:.4}",
        similarity(a, recovered).unwrap()
    );

    let store = Store::new(8192).unwrap();
    let _id1 = store
        .put("alpha".into(), b"alpha".to_vec(), vec!["test".into()])
        .unwrap();
